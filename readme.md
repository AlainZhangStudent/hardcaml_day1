...
2.  **Sequential Execution**: To match the software reference (Python), the hardware processes one "move" per clock cycle. When an `enable` pulse is received, the `number` of steps is loaded into `steps_remaining`.
